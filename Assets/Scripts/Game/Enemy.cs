...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
         ...
         */
        GameObject closest = GameManager.Instance.NearestPlayer(mRigidBody.transform.position);
        if (closest != null)
        {
            Vector3 t = mRigidBody.transform.position;
            Vector3 closePo = closest.transform.position;
            Vector3 followHere = new Vector3(closePo.x, t.y, closePo.z);
            mRigidBody.transform.position= Vector3.MoveTowards(t, followHere, speed * Time.deltaTime);
        }
    }
